...
public class SyncUtil {
     ...
     * 拼接源表主键where条件
     */
    public static String appendPkCondition(MappingConfig.DbMapping dbMapping, String sql, String backtick) {
        StringBuilder sqlBuilder = new StringBuilder(sql);
        // 拼接主键
        for (Map.Entry<String, String> entry : dbMapping.getSourcePk().entrySet()) {
            String sourceColumnName = entry.getKey();
            sqlBuilder.append(backtick).append(" WHERE ").append(sourceColumnName).append(backtick).append("=? AND ");
        }
